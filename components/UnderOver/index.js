...
import './underover.modules.css';
const UnderOver = ({ events }) => {
  const createTeamMatchesObject = (matches) => {
    ...
    matches.sort(sortByDate).forEach((match) => {
      ...
      const homeTeamState = {
        current: homeTeamScore,
        opposition: awayTeamScore,
        total: homeTeamScore + awayTeamScore
      }//homeTeamScore > awayTeamScore ? 'W' : homeTeamScore === awayTeamScore ? 'D' : 'L';
      const awayTeamState = {
        current: awayTeamScore,
        opposition: homeTeamScore,
        total: homeTeamScore + awayTeamScore
      }//homeTeamState === 'W' ? 'L' : homeTeamState === 'D' ? 'D' : 'W';
